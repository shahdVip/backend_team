...
router.post(
  "/addNewMember",
  auth([roles.Admin]),
  validation(schema.createMemberSchema),
  asyncHandler(authController.createMember)
);
router.put(
  "/updateMember/:id",
  auth([roles.Admin]),
  validation(schema.updateMemberSchema),
  asyncHandler(authController.updateMember)
);
router.get(
  "/getAllMembers",
  auth([roles.Admin]),
  asyncHandler(authController.getAllMembers)
);
router.delete(
  "/deleteMember/:id",
  auth([roles.Admin]),
  asyncHandler(authController.deleteMember)
);
const upload = multer({
  storage,
...
router.put(
  "/sendCode",
  validation(schema.sendCodeSchema),
...
router.patch(
  ...
  auth([roles.Admin]),
  asyncHandler(authController.toggleEmployeeStatus)
);
